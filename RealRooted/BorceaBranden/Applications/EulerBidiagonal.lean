...
/-!
# Euler affine bidiagonal operators

This module evaluates the genuine finite Borcea--Brändén symbol of the
Euler-style coefficient-bidiagonal operator, proves its upper-half-plane
stability for a diagonal constant `c ≥ 1`, and derives splitness and proper
position preservation on the corresponding degree box.

It is parameterized entirely by the operator data and has no OEIS model
dependency.
-/
  ...
  have htermder (k : ℕ) :
    ...
    | succ k =>
        ...
        rw [show MvPolynomial.C (k : ℝ) =
          (k : MvPolynomial (Fin 2) ℝ) by simp]
        ring
...
theorem eulerAffineBidiagonalResidualWithConstant_stable
  ...
  have hcombo :
      (z 1).im *
        ...
        (z 0).im *
          (((d : ℝ) + 1) * ((z 0).re ^ 2 + (z 0).im ^ 2) +
            2 * c * (z 0).re + ((d : ℝ) + 1) * c + c ^ 2 - c) = 0 := by grind
  have hdenpos :
      0 < (((d : ℝ) + 1) * (z 0).re + c) ^ 2 +
        (((d : ℝ) + 1) * (z 0).im) ^ 2 := by
...
theorem eulerBidiagonalStepWithConstant_splits
    {c : ℝ} (hc : 1 ≤ c) {d : ℕ} (hd : 1 ≤ d) {p : ℝ[X]}
    (hpdeg : p.natDegree ≤ d) (_hpPos : HasPosLeadingCoeff p)
