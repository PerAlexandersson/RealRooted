...
lemma roots_nonpos_coloredSetPartitions_of_isRealRooted {c m n : Nat}
    (hrr : (coloredSetPartitions c m n) ≠ 0 ∧ (coloredSetPartitions c m n).roots.card = (coloredSetPartitions c m n).natDegree) :
...
lemma prec_coloredSetPartitions_one_two (c m : Nat) :
  ...
  have hdeg : (coloredSetPartitions c m 1).natDegree = 1 := by
    simpa using natDegree_coloredSetPartitions c m 1
  have hf : ((coloredSetPartitions c m 1) ≠ 0 ∧ (coloredSetPartitions c m 1).roots.card = (coloredSetPartitions c m 1).natDegree) := isRealRooted_of_degree_one hdeg
  have hInter :
...
theorem prec_coloredSetPartitions_succ (c m : Nat) :
  ...
  | n + 2 => by
      ...
      have hf : ((coloredSetPartitions c m (n + 2)) ≠ 0 ∧ (coloredSetPartitions c m (n + 2)).roots.card = (coloredSetPartitions c m (n + 2)).natDegree) := hprev.2.1
...
theorem isRealRooted_coloredSetPartitions (c m : Nat) :
    ∀ n : Nat, ((coloredSetPartitions c m n) ≠ 0 ∧ (coloredSetPartitions c m n).roots.card = (coloredSetPartitions c m n).natDegree)
  | 0 => by
...
theorem isRealRooted_typeBSetPartitions (n : Nat) :
    ((typeBSetPartitions n) ≠ 0 ∧ (typeBSetPartitions n).roots.card = (typeBSetPartitions n).natDegree) :=
