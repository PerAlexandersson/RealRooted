...
lemma roots_nonpos_stirlingPermutations_of_isRealRooted {n : Nat}
    (hrr : (stirlingPermutations n) ≠ 0 ∧ (stirlingPermutations n).roots.card = (stirlingPermutations n).natDegree) :
...
lemma prec_stirlingPermutations_one_two :
  ...
  have hf : ((stirlingPermutations 1) ≠ 0 ∧ (stirlingPermutations 1).roots.card = (stirlingPermutations 1).natDegree) := isRealRooted_of_degree_one hdeg
  have hInter :
...
theorem prec_stirlingPermutations_succ :
  ...
  | n + 2 => by
      have hf : ((stirlingPermutations (n + 2)) ≠ 0 ∧ (stirlingPermutations (n + 2)).roots.card = (stirlingPermutations (n + 2)).natDegree) :=
...
theorem isRealRooted_stirlingPermutations :
    ∀ n : Nat, ((stirlingPermutations n) ≠ 0 ∧ (stirlingPermutations n).roots.card = (stirlingPermutations n).natDegree)
  | 0 => by
