...
lemma prec_one_sub_X_derivative_right {f : ℝ[X]}
    (hf : f ≠ 0 ∧ f.roots.card = f.natDegree) (hdeg : 2 ≤ f.natDegree) (hnn : HasNonnegCoeffs f) :
    Prec f ((1 - X) * f.derivative) := by
  have hder : Prec f.derivative f := (derivative_interlaces hf hdeg).toPrec
  have hf' : (f.derivative ≠ 0 ∧ f.derivative.roots.card = f.derivative.natDegree) := hder.1
  ...
  have hdeg' : f.derivative.natDegree + 1 = f.natDegree := by
    rw [natDegree_derivative_eq (by lia)]
    lia
...
lemma roots_nonpos_sturmDerangementsExc_of_isRealRooted {n : Nat} (_hn : 2 ≤ n)
    (hrr : (sturmDerangementsExc n) ≠ 0 ∧ (sturmDerangementsExc n).roots.card = (sturmDerangementsExc n).natDegree) :
...
theorem prec_sturmDerangementsExc_succ : ∀ n : Nat, 2 ≤ n →
  ...
  | 1, hn => by lia
  | 2, _ => by
      have hrr2 : ((sturmDerangementsExc 2) ≠ 0 ∧ (sturmDerangementsExc 2).roots.card = (sturmDerangementsExc 2).natDegree) := by
...
theorem isRealRooted_sturmDerangementsExc : ∀ n : Nat, 2 ≤ n →
    ((sturmDerangementsExc n) ≠ 0 ∧ (sturmDerangementsExc n).roots.card = (sturmDerangementsExc n).natDegree)
  | 0, hn => by lia
  | 1, hn => by lia
  | 2, _ => by
      apply isRealRooted_of_degree_one
