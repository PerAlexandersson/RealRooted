...
theorem prec_sub_X_mul_left {f g : ℝ[X]}
  ...
  set q : ℝ[X] := f - X * g
  by_cases hq0 : q = 0
  · simpa [q, hq0] using interl_zero_left f
...
theorem prec_sub_X_mul_pair_of_eq_posLeadingCoeff {f g : ℝ[X]}
  ...
  have hleft_scaled : StrictInterl (C c * (f - X * g)) (C c * f) := by
    have hEq : C c * f - X * (C c * g) = C c * (f - X * g) := by grind
...
/-- A positive right shear preserves `StrictInterl` when a larger scaled subtraction
...
theorem prec_right_shear_of_scaled_cancellation {f g : ℝ[X]} (c : ℝ)
  ...
  have hcancel_left : C c⁻¹ * (C c * g) = g := by
    calc
      C c⁻¹ * (C c * g) = C (c⁻¹ * c) * g := by grind
      _ = g := by simp [hc_ne]
  have hleft : StrictInterl g (f - X * (C c * g)) := by rwa [hcancel_left] at hleft_scaled
