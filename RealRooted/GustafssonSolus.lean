...
/-- Monotone thresholds and the no-switch hypotheses from Gustafsson--Solus Lemma 3.4
imply the concrete `2 × 2` affine-minor condition for this matrix. -/
theorem GustafssonSolus2x2FromNoSwitch {m n : ℕ} (phi : Fin m → Fin n) (dropPivot : Fin m → Bool)
    (hphi : GustafssonSolusWeaklyIncreasing phi)
    (hdrop : GustafssonSolusNoSwitchAfterDrop phi dropPivot) :
...
theorem gustafssonSolusLemma34 : GustafssonSolusLemma34Statement := by
  ...
  exact gustafssonSolus_preserves_interlacingSeq0Nonneg_of_2x2 hphi
    (GustafssonSolus2x2FromNoSwitch phi dropPivot hphi hdrop) fs hfs_len hfs
