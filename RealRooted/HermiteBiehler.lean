...
theorem interlaces_dup_mem {ss rs : List ℝ} (hrs : rs.Pairwise (· ≤ ·))
    (hint : ListInterlaces ss rs) (hnd : ¬ rs.Nodup) : ∃ r ∈ rs, r ∈ ss := by
  induction ss generalizing rs with
  | nil =>
    match rs with
    | [] => simp at hnd
  ...
  | cons s ss ih =>
    ...
    | r₁ :: r₂ :: rest =>
      ...
      · have h₂' : s ≤ r₁ := h_r₁_r₂ ▸ h₂
        ...
        simp [*]
      · have h_r₁_lt : r₁ < r₂ := lt_of_le_of_ne (h_r₁_le r₂ (by simp)) h_r₁_r₂
...
theorem alternates_dup_mem {ss rs : List ℝ} (hrs : rs.Pairwise (· ≤ ·))
    (halt : ListAlternates ss rs) (hnd : ¬ rs.Nodup) : ∃ r ∈ rs, r ∈ ss := by
  match ss, rs with
  | [], [] => simp at hnd
  | [], r :: rs' => simp [ListAlternates] at halt
  | s :: ss', [] => simp at hnd
...
theorem exists_common_root_of_not_nodup {f g : ℝ[X]} (hpq : Prec g f)
    (hnd : ¬ f.roots.Nodup) : ∃ r, r ∈ f.roots ∧ r ∈ g.roots := by
  obtain ⟨⟨hg₀, hgs⟩, ⟨hf₀, hfs⟩, ss, rs, hss, hrs, hsseq, hrseq, hshape⟩ := hpq
...
theorem interlaces_dup_mem_left {ss rs : List ℝ} (hss : ss.Pairwise (· ≤ ·))
    (hint : ListInterlaces ss rs) (hnd : ¬ ss.Nodup) : ∃ s ∈ ss, s ∈ rs := by
  induction ss generalizing rs with
  | nil => simp at hnd
  | cons s ss' ih =>
    match rs with
    ...
    | r₁ :: r₂ :: rest =>
      ...
      · obtain ⟨s₀, ss₀, hss₀eq⟩ : ∃ s₀ ss₀, ss' = s₀ :: ss₀ := by
        ...
        simp_all
      · have h_dup' : ¬ ss'.Nodup := fun hn ↦ hnd (List.nodup_cons.mpr ⟨h_s_mem, hn⟩)
...
theorem alternates_dup_mem_left {ss rs : List ℝ} (hss : ss.Pairwise (· ≤ ·))
    (halt : ListAlternates ss rs) (hnd : ¬ ss.Nodup) : ∃ s ∈ ss, s ∈ rs := by
  match ss, rs with
  | [], [] => simp at hnd
  | s :: ss', [] => simp [ListAlternates] at halt
  | [], r :: rs' => simp at hnd
...
theorem exists_common_root_of_not_nodup_g {f g : ℝ[X]} (hpq : Prec g f)
    (hnd : ¬ g.roots.Nodup) : ∃ r, r ∈ f.roots ∧ r ∈ g.roots := by
  obtain ⟨⟨hg₀, hgs⟩, ⟨hf₀, hfs⟩, ss, rs, hss, hrs, hsseq, hrseq, hshape⟩ := hpq
