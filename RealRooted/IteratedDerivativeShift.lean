...
/-- `T_ε p = p - ε p'` preserves real-rootedness for every real `ε`. -/
...
lemma derivative_common_factor_quotient_not_isRoot_of_not_dvd_X_sub_C
  ...
  intro hroot
  have hq_eval : q.eval a ≠ 0 := eval_ne_zero_of_not_dvd_X_sub_C hq
...
If `p` is real-rooted with `p(a) ≠ 0`, then
`rootMultiplicity a (T_ε p) ≤ 1` for every `ε`.
...
/-- Backward chain: if `rootMultiplicity a (TDeriv eps p) ≥ 2`, `eps ≠ 0`,
and `p` is real-rooted, then `rootMultiplicity a p` equals
...
lemma rootMultiplicity_eq_succ_of_TDeriv_ge_two
  ...
  have hroot : p.IsRoot a := by
    by_contra ha
    have := rootMultiplicity_TDeriv_le_one_of_not_isRoot (eps := eps) hp ha
    lia
...
/-- A nonzero derivative shift preserves the property of having simple roots
away from one exceptional point.  Multiple roots of `T_ε p` can only come from
multiple roots of `p`, with multiplicity lowered by one. -/
lemma hasSimpleRootsExcept_TDeriv
  ...
  have hx_mult_ge : 2 ≤ (TDeriv eps p).rootMultiplicity x := by
    lia
  have hsucc := rootMultiplicity_eq_succ_of_TDeriv_ge_two heps hp hx_mult_ge
  have hp_root : p.IsRoot x := by
...
theorem hasSimpleRoots_iterateTDeriv_of_natDegree
  ...
  have step : ∀ j, j ≤ n →
    ...
    | succ j ih =>
      ...
      have := rootMultiplicity_eq_succ_of_TDeriv_ge_two (ne_of_gt heps)
        (splits_iterateTDeriv heps hf)
        (by linarith)
      lia
