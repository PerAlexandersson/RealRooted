...
noncomputable def degreeOfLE (σ : Type*) (R : Type*) [CommSemiring R] (κ : σ → ℕ) :
...
theorem mem_degreeOfLE {κ : σ → ℕ} (p : MvPolynomial σ R) :
  ...
  simp only [degreeOfLE, mem_restrictSupport_iff, Set.subset_def, Set.mem_ofPred_eq,
    Finset.mem_coe]
...
theorem basisDegreeOfLE_repr_apply (κ : σ → ℕ)
    (p : degreeOfLE σ R κ)
    (m : {m : σ →₀ ℕ // ∀ i, m i ≤ κ i}) :
    (basisDegreeOfLE κ).repr p m = p.1.coeff m.1 :=
  by
    classical
    let coeffLinear : degreeOfLE σ R κ →ₗ[R] R :=
      { toFun := fun q : degreeOfLE σ R κ => q.1.coeff m.1
    ...
    have h :
      ...
      by_cases hmn : m = n
      · subst n
        change (((basisDegreeOfLE (R := R) κ m).val).coeff m.1) = _
        simp
      · have hnm' : n.1 ≠ m.1 := fun h => hmn (Subtype.ext h.symm)
        change (((basisDegreeOfLE (R := R) κ n).val).coeff m.1) = _
        simp [hmn, hnm']
