...
/-!
# Whitney reduction and oscillatory matrix core
...
private theorem whitney_adjacent_selector_identities {q : ℕ} {ι : Type*}
    (rows : Fin (q + 1) → ι) (r : Fin q) (pivot prior changed : ι)
  ...
  have hP : Function.update base r prior = r.succ.removeNth rows := by
    ...
    · simp [Fin.removeNth, Function.update, hprior]
  ...
  have hE : Function.update base r changed = r.castSucc.removeNth rows := by
    ...
    · have hir' : i.castSucc < r.succ :=
      ...
      simp [base, replaced, Fin.removeNth,
        ...
        ne_of_lt hir, ne_of_lt hir']
    · simp [Fin.removeNth, Function.update, hchanged]
    · have hne : i ≠ r := ne_of_gt hri
      have hne' : i.succ ≠ r.succ := by simpa
      simp [base, replaced, Fin.removeNth,
  ...
  have hDC : r.castSucc.insertNth prior base = replaced := by
    apply Fin.insertNth_eq_iff.2
    constructor
    · simp [replaced, hpositions, hprior]
    · rfl
          ...
          · let r : Fin q := p.pred hp0
            ...
            have hqrows : StrictMono qrows := by
              ...
              · intro i hi
                ...
                exact lt_of_lt_of_le (by simp [hp']) hmono
...
private theorem whitneyClearColumnAux_spec {N : ℕ}
  ...
  | succ t ih =>
      ...
      rw [whitneyClearColumnAux, dite_eq_left htActive]
      dsimp only
...
private theorem whitneyClearColumnLowerAux_spec {N : ℕ}
  ...
  | succ t ih =>
      ...
      have hlower : whitneyClearColumnLowerAux A c (t + 1) =
        ...
        rw [whitneyClearColumnLowerAux, dite_eq_left htActive]
      rw [hclear, hlower]
      by_cases hzero : B s.succ c = 0
      ...
      · let s : Fin N := ⟨N - (t + 1), by lia⟩
        ...
        · rw [ite_eq_right hzero]
          ...
          · change whitneyEliminateAt B s c i j = 0
            ...
            exact ih i j hij
      · rw [whitneyClearColumnAux, dite_eq_right htActive]
        exact ih i j hij
...
private theorem whitneyClearLowerAux_spec {N : ℕ}
  ...
  | succ t ih =>
      ...
      have hlower : whitneyClearLowerAux A (t + 1) =
        ...
        rw [whitneyClearLowerAux, dite_eq_left htN]
      rw [hclear, hlower]
      by_cases hzero : B s.succ 0 = 0
      · rw [ite_eq_left hzero, ite_eq_left hzero]
        exact ⟨hL, hfactor⟩
...
private theorem whitneyClearLowerAux_first_block {N : ℕ}
  ...
  | succ t ih =>
      ...
      · rw [whitneyClearLowerAux, dite_eq_left htN, ite_eq_left hzero]
        exact ⟨hcol, hrow⟩
      · rw [whitneyClearLowerAux, dite_eq_left htN, ite_eq_right hzero]
...
private theorem whitneyClearLowerAux_unitLower {N : ℕ}
  ...
  | succ t ih =>
      ...
      · let s : Fin N := ⟨N - (t + 1), by lia⟩
        let B := whitneyClearFirstAux A t
        let L := whitneyClearLowerAux A t
        rw [whitneyClearLowerAux, dite_eq_left htActive]
        by_cases hzero : B s.succ 0 = 0
        · rw [ite_eq_left hzero]
          exact ih
        · rw [ite_eq_right hzero]
          ...
          · intro i
            ...
            · rw [Matrix.mul_transvection_apply_of_ne
              ...
              exact ih.2 i
      · rw [whitneyClearLowerAux, dite_eq_right htActive]
        exact ih
...
private theorem whitneyClearColumnCycle_adjacent_pos {N : ℕ}
  ...
  | succ t ih =>
      ...
      rw [whitneyClearColumnAux, whitneyClearColumnLowerAux,
        dite_eq_left htActive, dite_eq_left htActive]
      dsimp only
      by_cases hzero : B s.succ c = 0
      · rw [ite_eq_left hzero, ite_eq_left hzero]
...
/-- A real tridiagonal matrix with positive adjacent off-diagonal entries has
a Hermitian diagonal-similarity model with the same full and trailing
characteristic polynomials. Its consecutive trailing characteristic
polynomials have no common root. -/
theorem exists_hermitianModel_of_tridiagonal {N : ℕ}
    (T : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ)
    (hlower : ∀ r k, k.val + 1 < r.val → T r k = 0)
    (hupper : ∀ i j, i.val + 1 < j.val → T i j = 0)
    (hsuper : ∀ i : Fin N, 0 < T i.castSucc i.succ)
    ...
    ∃ S : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ,
      S.IsHermitian ∧
        S.charpoly = T.charpoly ∧
        (S.submatrix Fin.succ Fin.succ).charpoly =
          (T.submatrix Fin.succ Fin.succ).charpoly ∧
        ∀ x : ℝ, ¬(T.charpoly.IsRoot x ∧
          (T.submatrix Fin.succ Fin.succ).charpoly.IsRoot x) := by
  let S := symmetrizeTridiagonal T
  have hdpos := tridiagonalScale_pos T hsuper hsub
  have hdne : ∀ i, tridiagonalScale T i ≠ 0 := fun i ↦ (hdpos i).ne'
  exact ⟨S,
    symmetrizeTridiagonal_isHermitian T hlower hupper hsuper hsub,
