...
theorem MvPolynomial.IsMultiaffine.specializeAt_eq_specializeZero_add_C_mul_pderiv
  ...
  classical
  have hiZero : i ∉ (MvPolynomial.specializeZero i P).vars := by
    intro hi
    exact (Finset.mem_erase.mp
      (MvPolynomial.vars_specializeZero_subset_erase P i hi)).1 rfl
  conv_lhs => rw [hP.eq_specializeZero_add_X_mul_pderiv i]
  rw [MvPolynomial.specializeAt_add,
    MvPolynomial.specializeAt_eq_of_notMem_vars hiZero,
...
/-- Specializing a multiaffine coordinate at one adds its zero-section and
its partial derivative. -/
theorem MvPolynomial.IsMultiaffine.specializeAt_one_eq_specializeZero_add_pderiv
    {σ S : Type*} [CommRing S] {P : MvPolynomial σ S}
    (hP : P.IsMultiaffine) (i : σ) :
    MvPolynomial.specializeAt i 1 P =
      MvPolynomial.specializeZero i P + MvPolynomial.pderiv i P := by
