...
theorem preservesAllComboPairs_of_preservesRealRootedOrZero
  ...
  · left
    simpa [hmap] using congrArg T hzero
  · rcases hT (C α * f + C β * g) hrr with hzeroT | hrrT
    · left
      simpa [hmap] using hzeroT
    · right
      simpa [hmap] using hrrT
