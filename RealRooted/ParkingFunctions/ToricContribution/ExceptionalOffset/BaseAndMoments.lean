...
/-!
# Exceptional-offset base polynomials and moment identities

This layer defines the Euler-inverse family used when the finite derivative
triangle drops degree. It proves its endpoint product formula, coefficient
control, and the Jacobi and exterior moment identities used by the pencil
argument.
-/
...
/-- Rising factorial at a negative natural, in quotient form. This package
helper is shared by the endpoint and Jacobi-node layers. -/
theorem exceptional_realRisingFactorial_neg_nat_eq_factorial_div
    (m r : ℕ) (hr : r ≤ m) :
    realRisingFactorial (-(m : ℝ)) r =
...
/-- The square of a real parity sign is one. -/
theorem exceptional_neg_one_pow_sq_real (k : ℕ) :
    ((-1 : ℝ) ^ k) ^ 2 = 1 := by
  rw [← pow_mul]
  norm_num
...
private theorem exceptionalEndpointNumerator_eval_neg_nat
  ...
  rw [exceptional_realRisingFactorial_neg_nat_eq_factorial_div m r hr,
  ...
  have hsignR : (-1 : ℝ) ^ (r * 2) = 1 := by
    rw [pow_mul, exceptional_neg_one_pow_sq_real]
...
theorem negOnePow_mul_exceptionalEulerInverse_eval_one_pos
  ...
  rw [exceptional_realRisingFactorial_neg_nat_eq_factorial_div m m le_rfl]
  simp only [Nat.sub_self, Nat.factorial_zero, Nat.cast_one, div_one]
...
theorem exceptionalBaseCoeff_ne_zero_of_le
  ...
  rw [exceptionalBaseCoeff,
    exceptional_realRisingFactorial_neg_nat_eq_factorial_div m k hk]
  have hc : 0 < (ε : ℝ) + 1 / 2 := by positivity
