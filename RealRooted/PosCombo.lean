...
lemma prec_of_prec_mul_X_sub_C_of_sameDegree_of_roots_le {f g : ℝ[X]} (r : ℝ)
  ...
  set f' := f.comp (X + C r)
  set g' := g.comp (X + C r)
  have hXf : (((X - C r) * f) ≠ 0 ∧ ((X - C r) * f).roots.card = ((X - C r) * f).natDegree) := h.2.1
...
interlaced on the left by `h`. -/
  ...
  have hh : (h ≠ 0 ∧ h.roots.card = h.natDegree) := (hprec ap0 hap0).1
...
theorem prec_sum_left_of_common_left_signed
  ...
  have hh : (h ≠ 0 ∧ h.roots.card = h.natDegree) := (hprec p0 hp0).1
...
theorem prec0_sum_left_of_common_left_of_nonneg
  ...
  · have hprec' : ∀ p ∈ l', Prec h p := by
    ...
    have hpos' : ∀ p ∈ l', HasPosLeadingCoeff p := by
      ...
      have hp_ne : p ≠ 0 := by
        simpa using (List.mem_filter.mp hp).2
      have hp_rr : (p ≠ 0 ∧ p.roots.card = p.natDegree) := (hprec' p hp).2.1
...
theorem isRealRooted_nonneg_combo_of_prec {f g : ℝ[X]}
    ...
    {a b : ℝ} (ha : 0 ≤ a) (hb : 0 ≤ b)
    (hab : 0 < a ∨ 0 < b) :
    ((C a * f + C b * g) ≠ 0 ∧ (C a * f + C b * g).roots.card = (C a * f + C b * g).natDegree) :=
...
lemma isRealRooted_add {f g : ℝ[X]} (h : PosComboRealRooted f g) :
    ((f + g) ≠ 0 ∧ (f + g).roots.card = (f + g).natDegree) := by
