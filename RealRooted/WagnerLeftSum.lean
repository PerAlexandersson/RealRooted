...
private lemma wagner2_roots_exist (f g : ℝ[X])
  ...
  | rf, rg, [], [], [], _, _, _, _, hrf_eq, hrg_eq,
    ...
    have hf_roots : f.roots = rf ::ₘ consumed_f := by simp_all
    have hg_roots : g.roots = rg ::ₘ consumed_g := by simp_all
    have hf_erase : f.roots.erase rf = consumed_f := by simp_all
    have hg_erase : g.roots.erase rg = consumed_g := by simp_all
    -- Use sign lemma with a = min(rf,rg), b = max(rf,rg) + 1
    have hab : min rf rg ≤ max rf rg + 1 := by grind
    ...
    · have hsign :=
      ...
      exact ⟨[u], rfl, trivial, fun v hv => by simp_all,
        List.pairwise_singleton _ _, fun v hv => by simp_all⟩
