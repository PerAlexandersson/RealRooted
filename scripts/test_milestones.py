...
def fixture():
    return {"schema_version": 1, "milestones": [{
        ...
        "witnesses": [{
            "module": "RealRooted.Challenges.Example",
            "declaration": "RealRooted.Challenges.Example.Foo.bar",
        }],
    }]}
...
class CatalogTests(unittest.TestCase):
    ...
    def test_malformed_milestone_entry(self):
        data = fixture()
        data["milestones"] = ["not an object"]
        with self.assertRaisesRegex(ValueError, "entries must be objects"):
            validate_catalog(data)

    def test_malformed_witness_entry(self):
        data = fixture()
        data["milestones"][0]["witnesses"] = ["not an object"]
        with self.assertRaisesRegex(ValueError, "witness entries must be objects"):
            validate_catalog(data)
    ...
    def test_declaration_must_use_listed_module_namespace(self):
        data = fixture()
        data["milestones"][0]["witnesses"][0]["declaration"] = "Other.Foo"
        with self.assertRaisesRegex(ValueError, "listed challenge module"):
            validate_catalog(data)
    ...
    def test_source_only_expected_commands(self):
        ...
        self.assertIn("#print axioms RealRooted.Challenges.Example.Foo.bar\n", source)
        self.assertIn("some (.thmInfo _)", source)
        self.assertIn(
            "Not a theorem witness: RealRooted.Challenges.Example.Foo.bar", source
        )
    ...
    def test_apostrophe_in_declaration_name(self):
        name = "RealRooted.Challenges.Example.Foo'bar"
        result = parse_axioms(f"'{name}' does not depend on any axioms\n", [name])
